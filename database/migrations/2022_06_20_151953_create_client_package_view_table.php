...
class CreateClientPackageViewTable extends Migration
{
    ...
    private function createView(): string
    {
        return <<<SQL
                ...
                (SELECT package_discount as palatinum_discount ,users.fullname as name,users.id AS id  FROM client_package JOIN users ON users.id = client_package.vendor_id JOIN packages
                ...
                JOIN
                (SELECT package_discount as basic_discount ,users.id AS id  FROM client_package JOIN users ON users.id = client_package.vendor_id JOIN packages ON packages.id = client_package.package_id
                JOIN package_translations ON package_translations.package_id = packages.id WHERE package_translations.name = 'Basic' ) as q2
                ON q1.id = q2.id
                JOIN
                (SELECT package_discount as golden_discount ,users.id AS id  FROM client_package JOIN users ON users.id = client_package.vendor_id JOIN packages ON packages.id = client_package.package_id
