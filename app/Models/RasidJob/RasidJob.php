...
class RasidJob extends Model implements TranslatableContract
{
    ...
    public function scopeSearch(Builder $query, $request, $subProgram = 'index')
    {
        ...
        if ($old != $new | $request->is_active == -1 || $request->is_vacant == -1)
        ...
        $query->when($request->sort, function ($q) use ($request) {
            if ($request->sort["column"] == "name") {
                return $q->has('translations')
                    ->orderBy($request->sort["column"], @$request->sort["dir"])->latest('rasid_jobs.created_at');
            }
            ...
            if ($request->sort["column"] == "department") {
                return $q->join('departments as department', 'rasid_jobs.department_id', '=', 'department.id')
                    ...
                    ->orderBy('department_trans.name', @$request->sort["dir"])->latest('rasid_jobs.created_at');
            }

            $q->orderBy($request->sort["column"], @$request->sort["dir"])->latest('rasid_jobs.created_at');
        });
    }
