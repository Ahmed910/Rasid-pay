...
class TransactionMiddleware
{
    public function handle($request, Closure $next)
    {
        ...
        if ($dailyTransactions >= $maxTransactionPerDay) {
            return response()->json(['status' => false, 'message' => trans('mobile.transactions.transaction_details.reach_max_transaction_day'), 'data' => null], 422);
        }
        ...
        if ($monthlyTransactions >= $maxTransactionPerMonth) {
            return response()->json(['status' => false, 'message' => trans('mobile.transactions.transaction_details.reach_max_transaction_month'), 'data' => null], 422);
        }
        return $next($request);
    }
}
