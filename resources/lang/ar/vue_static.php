<?php

return  [
    "appName" => "رصيد ",
    "validation" => [
        "validPhoneNumber" => "أدخل رقم الهاتف بطريقة صحيحة",
        "required" => "%{attribute} مطلوب",
        "required_f" => "%{attribute} مطلوبة",
        "maxLength" => "%{attribute} يجب ان لا يتجاوز %{max} حروف ",
        "minLength" => "%{attribute} يجب ان يكون علي الاقل %{min} حروف",
        "minWords" => "%{attribute} يجب ألا يقل عن 3 كلمات",
        "maxWords" => "%{attribute} يجب أن يكون أقل من %{maxWords} كلمة",
        "between" => "القيمة يجب أن تكون بين %{min} و %{max}",
        "description" => "Description",
        "valid" => "يجب أن تكون صالحة",
        "sameAsPassword" => "كلمه المرور غير متطابقه ",
        "number" => " يجب أن تكون أرقام",
        "set_as_default_address" => "Set as default Address",
        "add_address" => "Add New address",
        "signout" => "Sign Out",
        "password_min" => "must be 6 numbers",
        "numeric" => "%{attribute} يجب أن يكون أرقام",
        "email" => "%{attribute} غير صحيح رجاء التأكد من الكتابة بطريقة صحيحة",
        "url" => " not valid Url",
        "file_type" => "يجب أن يكون نوع الملف pdf فقط",
        "file_size" => "File Size",
        "imageType" => "يجب أن يكون نوع الصورة jpg , png , jpeg",
        "imageSize" => "يجب أن يكون حجم الصورة أقل من %{count} kb",
        "imageDimensions" => "أبعاد الصورة غير صالحة",
        "image_ratio" => "نسبة الصورة غير صالحة",
        "isPngType" => "فقط ",
        "sameAs" => "%{attribute} يجب ان يكون مساوي ل %{eq}",
        "beforeDate" => "%{attribute} يجب ان يكون ان قبل  %{beforeDate}",
        "afterDate" => "%{attribute} يجب ان يكون بعد %{afterDate}",
        "allType" => "يجب أن يكون امتداد الملف pdf أو docs",
        "docsType" => "يجب أن يكون امتداد الملف doc أو DOC أو pdf أو PDF",
        "videoType" => "يجب أن يكون امتداد الملف فيديو بالصيغ التالية :   avi أو flv أو mov أو 3gp أو mkv أو rmvb أو wepm أو wmv أو mp4",
        "voiceType" => "يجب أن يكون امتداد الملف wav أو mp3",
        "noJobDepartment" => "لا يوجد وظائف لهذا القسم",
        "maxValue" => "%{attribute} يجب ان تكون اقل من %{max}",
        "minValue" => "%{attribute} يجب ان علي الاقل %{min}"
    ],
    "dropzone" => [
        "dictDefaultMessage" => "اسحب  واسقط أو قم برفع الصورة",
        "dictFallbackMessage" => "لا يدعم المستعرض الخاص بك تحميل الملفات بالسحب والإفلات",
        "dictFallbackText" => "يرجى استخدام النموذج الاحتياطي أدناه لتحميل ملفاتك كما في الأيام الخوالي",
        "dictFileTooBig" => "الملف كبير جدا ({{filesize}}MiB) أقصي حجم: {{maxFilesize}}MiB",
        "dictInvalidFileType" => "لا يمكنك تحميل ملفات من هذا النوع",
        "dictResponseError" => "استجاب الخادم برمز {{statusCode}}",
        "dictCancelUpload" => "إلغاء الرفع",
        "dictCancelUploadConfirmation" => "هل أنت متأكد أنك تريد إلغاء هذا التحميل؟",
        "dictMaxFilesExceeded" => "لا يمكنك تحميل المزيد من الملفات"
    ],
    "heading" => [
        "dashboard" => "Dashboard",
        "jobs" => "Jobs",
        "edit" => "تعديل",
        "create" => "Create",
        "add_department" => "add department",
        "add_job" => "أضف الوظيفة",
        "static_pages" => "Static Pages",
        "permissions" => "Permissions",
        "users" => "Users",
        "settings" => "Settings",
        "applicants" => "applicants",
        "jobCategories" => "job Categories",
        "departments" => "Departments",
        "archive" => "أرشيف",
        "main_information" => "المعلومات الرئيسية",
        "optional_information" => "معلومات اختيارية",
        "departments_archives" => "Departments Archives",
        "editDepartment" => "تعديل قسم",
        "createDepartment" => "إضافة قسم",
        "view_department" => "عرض القسم",
        "view_job" => "عرض الوظيفة",
        "editJob" => "تعديل وظيفة",
        "createJob" => "إضافة وظيفة",
        "login" => "تسجيل الدخول",
        "login_description" => "من فضلك أدخل رقم المستخدم وكلمة المرور",
        "reset_password" => "إعادة تعيين كلمة المرور؟",
        "verification" => "التحقق من المستخدم",
        "choose_verification_method" => "الرجاء اختيار طريقة التحقق",
        "verification_code" => "رمز التحقق",
        "verify_description" => "تم إرسال تحقق برمز تحقق للتو إلى",
        "historical_movment" => "الحركة التاريخية",
        "selected_permissions" => "الصلاحيات المختارة",
        "edit_user" => "تعديل مستخدم",
        "add_user" => "إضافة مستخدم",
        "disable_user" => "تعطيل المستخدم",
        "history" => "الحركة التاريخية",
        "groupData" => "بيانات المجموعة",
        "managers_names" => "أسماء المديرين/المفوضين",
        "createClient" => "إضافة عميل",
        "editClient" => "تعديل عميل",
        "reset_your_password" => "إعادة تعيين كلمة المرور",
        "add_phone_or_email" => "من فضلك أدخل رقم جوالك أو بريدك الإلكتروني لإرسال رمز التحقق"
    ],
    "message" => [
        "edit" => "Edit",
        "create" => "create",
        "static_pages" => "Static Pages",
        "settings" => "Settings",
        "users" => "Users",
        "permissions" => "Permissions",
        "warning" => "Are You Sure?",
        "not_found" => "Not Found",
        "no_data" => "No Data"
    ],
    "button" => [
        "language" => "اللغة",
        "login" => "تسجيل دخول",
        "close" => "إغلاق",
        "logout" => "تسجيل الخروج",
        "create" => "إنشاء",
        "edit" => "تعديل",
        "delete" => "حذف",
        "save" => "حفظ",
        "confirm" => "تأكيد",
        "cancel" => "إلغاء",
        "search" => "بحث",
        "got_it" => "تم",
        "add" => "إضافة",
        "reset" => "عرض الكل",
        "export" => "تصدير",
        "back" => "عودة",
        "view_details" => "مشاهدة التفاصيل",
        "restore" => "إسترجاع",
        "alert" => "تنبيه",
        "send" => "إرسال",
        "resendCode" => "إعادة إرسال الرمز مرة أخرى",
        "addDepartment" => "إضافة قسم",
        "addCategory" => "cancel",
        "go_home" => "اذهب للرئيسية",
        "agree" => "موافق",
        "not_agree" => "غير موافق",
        "accept" => "موافق",
        "not agree" => "غير موافق",
        "add_job" => "إضافة وظيفة",
        "addclient" => "تسجيل عميل",
        "print_report" => "طباعة تقرير",
        "continue" => "استكمال",
        "add-attach" => "إضافة مرفق"
    ],
    "labels" => [
        "search" => "Search",
        "Description" => "الوصف",
        "users" => "Users",
        "settings" => "Settings",
        "static_pages" => "Static Pages",
        "jobs" => "Jobs",
        "confirm" => "Yes",
        "cancel" => "Cancel",
        "go_home" => "Go Home",
        "tags" => "Tags",
        "facebook" => "Facebook",
        "twitter" => "Twitter",
        "instagram" => "Instagram",
        "linkedin" => "linkedIn",
        "whatsapp" => "whatsApp",
        "footer" => "Footer",
        "phone" => "رقم الجوال",
        "phone_0" => "second Phone",
        "jobCategories" => "job Categories",
        "choose_file" => "اختر صورة",
        "upload_image" => "تحميل الصور",
        "show_image" => "show image",
        "email" => "البريد الإلكتروني",
        "enter_password" => "أدخل كلمةالمرور",
        "enter_email" => "أدخل البريد الإلكتروني",
        "applicants" => "Applicants",
        "users_view" => "Users View",
        "users_create" => "Users Create",
        "users_edit" => "Users Edit",
        "users_delete" => "Users delete",
        "users_show" => "Users Show",
        "jobs_view" => "Job View",
        "jobs_create" => "Job create",
        "jobs_edit" => "Job edit",
        "jobs_delete" => "Job delete",
        "jobs_show" => "Job show",
        "job_name" => "اسم الوظيفة",
        "employee_name" => "اسم الموظف",
        "settings_view" => "Settings View",
        "settings_create" => "Settings create",
        "settings_edit" => "Settings edit",
        "settings_delete" => "Settings delete",
        "settings_show" => "Settings show",
        "static_pages_view" => "Static Pages View",
        "static_pages_create" => "Static Pages create",
        "static_pages_edit" => "Static Pages edit",
        "static_pages_delete" => "Static Pages delete",
        "static_pages_show" => "Static Pages show",
        "jobs_categories_view" => "job categories view",
        "jobs_categories_create" => "job categories create",
        "jobs_categories_edit" => "job categories edit",
        "jobs_categories_delete" => "job categories delete",
        "jobs_categories_show" => "job categories show",
        "applicants_create" => "applicants create",
        "applicants_view" => "applicants view",
        "applicants_delete" => "applicants delete",
        "applicants_edit" => "applicants edit",
        "applicants_show" => "applicants show",
        "department_name" => "اسم القسم ",
        "enter_name" => "أدخل الاسم",
        "creating_date" => "تاريخ الإنشاء (من)",
        "end_date" => "تاريخ الإنشاء (إلى)",
        "archive_date" => "تاريخ الأرشفة",
        "archive_date_form" => "تاريخ الأرشفة (من)",
        "archive_date_to" => "تاريخ الأرشفة (إلي)",
        "select_date" => "يوم/شهر/سنة",
        "select_department" => "اختر القسم",
        "main_department" => "القسم الرئيسي",
        "department" => "القسم",
        "all_departemnets" => "جميع الأقسام",
        "status" => " الحالة",
        "department_image" => "صورة القسم",
        "description" => "الوصف",
        "image" => "صورة",
        "drag_image" => "اسحب  واسقط أو قم برفع الصورة",
        "type" => "النوع",
        "select_type" => "اختر النوع",
        "depatmentimage" => "صورة القسم",
        "userID" => "رقم المستخدم",
        "enter_userID" => "أدخل رقم المستخدم الخاص بك",
        "password" => "كلمة المرور",
        "remember" => "تذكرني",
        "new_password" => "كلمة المرور الجديدة",
        "confirm_password" => "تأكيد كلمة المرور",
        "mobile_number" => "رقم الجوال",
        "mobile" => "رقم الجوال",
        "permission_name" => "اسم الصلاحية",
        "permission_user_from" => "عدد المستخدمين (من)",
        "permission_user_to" => "عدد المستخدمين (إلى)",
        "user_name" => "اسم المستخدم",
        "enter_user_number" => "أدخل رقم المستخدم",
        "user_number" => "رقم المستخدم",
        "user_status" => "الحالة",
        "verfication_code" => "إرسال رمز التحقق",
        "choose_department" => "اختر القسم",
        "employee" => "الموظف",
        "select_employee" => "اختر موظف",
        "add_user_number" => "أدخل رقم المستخدم",
        "send_activity_code" => "إرسال رمز التحقق",
        "system_permissions" => "صلاحيات النظام",
        "select_permissions" => "اختر الصلاحيات",
        "roles" => "مجموعة صلاحيات",
        "check_password" => "كلمة المرور",
        "select_roles" => "حدد الأدوار",
        "select_status" => "اختر الحالة",
        "to" => "إلى",
        "from" => "من",
        "enter_confirmation_password" => "أدخل تأكيد كلمة المرور",
        "enter_phoneNumber" => "أدخل رقم الجوال",
        "temporary" => "لفترة",
        "permanent" => "دائم",
        "group_name" => "اسم المجموعة",
        "period_from" => "فترة (من)",
        "period_to" => "فترة (إلى)",
        "permanent_status" => "معطل دائم",
        "temporary_status" => "معطل لفترة",
        "of" => "من",
        "result" => "الإجمالي",
        "reason" => "الرجاء ذكر السبب*",
        "groupManger" => "منشئ المجموعة",
        "bank" => "البنك",
        "account_status" => "حالة الحساب البنكي",
        "transactions_form" => "عدد المعاملات من",
        "transactions_to" => "عدد المعاملات إلى",
        "client_period_from" => " الفترة من",
        "client_period_to" => "الفتره إلى",
        "order_number" => "رقم الطلب",
        "client_name" => "اسم العميل",
        "client_type" => "نوع العميل",
        "record_number" => "رقم السجل التجاري",
        "tax_number" => "الرقم الضريبي",
        "account_number" => "رقم الحساب البنكي",
        "activity_type" => "نوع النشاط",
        "client_address" => "العنوان",
        "client_nationality" => "الجنسية",
        "client_gender" => "النوع",
        "client_social_status" => "الحالة الاجتماعية",
        "bank_name" => "اسم البنك",
        "order_data_form" => "تاريخ الطلب من",
        "order_data_to" => "تاريخ الطلب إلى",
        "chooseBank" => "إختر البنك",
        "chooseClient" => "إختر نوع العميل",
        "job_description" => "الوصف الوظيفي",
        "enter_description" => "أدخل الوصف ",
        "job_desription" => "الوصف الوظيفي",
        "client_attatchments" => "مرفقات العميل",
        "identity_card" => "بطاقة الهوية",
        "register_type" => "نوع التسجيل",
        "daily_transactions" => "إجمالي المعاملات اليومية المتوقعة",
        "manager_data" => "بيانات المدير",
        "manager_name" => "اسم المدير",
        "select_companies" => "شركات",
        "select_register" => "اختر التسجيل",
        "select_bank" => "اختر البنك",
        "identity_number" => "رقم الهوية",
        "select_activity_name" => "اختر النشاط",
        "activity_name" => "اسم النشاط",
        "choose_employee_name" => "أدخل اسم الموظف",
        "choose_main_program" => "اختر البرنامج الرئيسي",
        "choose_sub_program" => "اختر البرنامج الفرعي",
        "main_program" => "البرنامج الرئيسي",
        "sub_program" => "البرنامج الفرعي",
        "birth_date" => "تاريخ الميلاد",
        "jawal" => "الجوال",
        "attachments" => "المرفقات",
        "attachment-type" => "نوع المرفق",
        "attachement-address" => "عنوان المرفق",
        "choose_status" => "اختر الحالة",
        "choose_type" => "اختر النوع",
        "transactions_done" => "عدد المعاملات المنجزة",
        "bank_account_status" => "حالة الحساب البنكي",
        "daily-trans" => "قيمة المعاملات اليومية",
        "choose_job_name" => "أدخل اسم الوظيفة",
        "enter__job_name" => "أدخل اسم الوظيفة",
        "select_main_department" => "اختر القسم الرئيسي",
        "add_description" => "أدخل الوصف",
        "enter__user_name" => "أدخل اسم المستخدم",
        "not-found" => "لا يوجد",
        "enter_number" => "أدخل الرقم",
        "attatchments" => "المرفقات",
        "address" => "العنوان",
        "gender" => "النوع",
        "marital_status" => "الحالة الاجتماعية",
        "nationality" => "الجنسية",
        "contract_type" => "نوع العقد",
        "transaction_done" => "عدد المعاملات المنجزة",
        "activated_card" => "البطاقة المفعلة",
        "card_endDate_from" => "تاريخ إنتهاء البطاقة (من)",
        "card_endDate_to" => "تاريخ إنتهاء البطاقة (إلى)",
        "register_date_from" => "تاريخ التسجيل (من)",
        "register_date_to" => "تاريخ التسجيل (إلى)",
        "new_phone" => "رقم الجوال الجديد",
        "transaction_number" => "رقم المعاملة",
        "enter-transaction" => "أدخل رقم المعاملة",
        "enter_identity_number" => "أدخل رقم الهوية",
        "client" => "العميل",
        "choose_client" => "اختر العميل",
        "transaction_date_from" => "تاريخ المعاملة (من)",
        "transaction_date_to" => "تاريخ المعاملة (إلى)",
        "transaction_value_from" => "قيمة المعاملة (من)",
        "transaction_value_to" => "قيمة المعاملة (إلى)",
        "transaction_value" => "قيمة المعاملة",
        "transaction_type" => "نوع المعاملة",
        "choose_transaction_type" => "اختر نوع المعاملة",
        "enter_transaction_value" => "أدخل قيمة المعاملة",
        "active_identity_card" => "البطاقة المفعلة",
        "choose_identity_type" => "اختر نوع البطاقة",
        "transaction_status" => "حالة المعاملة",
        "choose_transaction_status" => "اختر حالة المعاملة",
        "receipt_value" => "قيمة الفاتورة",
        "earned_rewards" => "المكافآت المكتسبة",
        "identity_discount_rate" => "نسبة خصم البطاقة",
        "enter_bank_name" => "أدخل اسم البنك",
        "choose_bank_type" => "اختر نوع البنك",
        "enter_code" => "أدخل الكود",
        "enter_branch_name" => "أدخل اسم الفرع",
        "enter_location" => "أدخل الموقع",
        "enter_transfer_cost" => "أدخل قيمة التكلفة",
        "branch_name" => "اسم الفرع",
        "location" => "الموقع",
        "code" => "الكود",
        "transfer_cost" => "قيمة تكلفة التحويل",
        "transaction_count" => "عدد المعاملات",
        "enter_transaction_count" => "أدخل عدد المعاملات",
        "select_bank_type" => "اختر نوع البنك",
        "commercial_record" => "السجل التجاري",
        "enter_commercial_record" => "أدخل السجل التجاري",
        "enter_tax_number" => "أدخل الرقم الضريبي",
        "customer_service_number" => "رقم خدمة العملاء",
        "enter_customer_service_number" => "أدخل رقم خدمة العملاء",
        "card_endDate" => "تاريخ انتهاء صلاحية البطاقة",
        "form" => "من عند",
        "basic_card" => "نسبة خصم البطاقة الأساسية",
        "add_discount_percentage" => " أدخل نسبة الخصم",
        "add_identity_number" => "أدخل رقم الهوية",
        "all" => "الجميع",
        "active_card" => "البطاقة المفعلة",
        "service_customer" => "رقم خدمة العملاء",
        "gold_card" => "نسبة خصم البطاقة الذهبية",
        "platinum_card" => "نسبة خصم البطاقة البلاتينية",
        "archive_date_from" => "تاريخ الأرشفة (من)",
        "enter_phone" => "أدخل رقم الجوال",
        "select_job_name" => "أختر الوظيفة",
        "job" => "الوظيفة",
        "statistics" => "الإحصائيات",
        "add_number" => "أدخل الرقم",
        "choose_client_name" => "اختر اسم العميل",
        "country_code" => "الرقم الدولي",
        "enabled_card" => "البطاقة المفعلة",
        "card_end_at" => "تاريخ انتهاء البطاقة",
        "register_date" => "تاريخ التسجيل",
        "profile" => "الملف الشخصي",
        "invoice_amount" => "قيمة الفتورة",
        "enabled_package_discount" => "نسبة خصم البطاقة",
        "amount" => "قيمة المعاملة",
        "cash_back" => "المكافئات المكتسبة"
    ],
    "fields" => [
        "email" => "البريد الإلكتروني",
        "password" => "كلمة المرور",
        "jobName" => "اسم الوظيفة",
        "title_en" => "العنوان بالإنجليزية",
        "title_ar" => "العنوان بالعربية",
        "content_en" => "المحتوي بالإنجليزية",
        "content_ar" => "المحتوي بالعربية",
        "status" => "الحالة",
        "name" => "الاسم",
        "description" => "الوصف",
        "title" => "العنوان",
        "location" => "الموقع",
        "category_job_id" => "القسم",
        "footer" => "التذييل",
        "description_arabic" => "الوصف بالعربية",
        "footer_arabic" => "التذييل بالعربية",
        "reason" => "السبب",
        "category_id" => "القسم",
        "userID" => "رقم المستخدم",
        "mobile" => "رقم الجوال",
        "employee" => "الموظف",
        "user_number" => "رقم المستخدم",
        "parent" => "اختر القسم الرئيسي",
        "confirm_password" => "تأكيد كلمة المرور",
        "ban_from" => "تاريخ من",
        "ban_to" => "تاريخ إلى",
        "chooseStatus" => "اختر الحالة",
        "department" => "القسم",
        "user" => "مستخدم",
        "permissions_list" => "صلاحيات النظام",
        "from" => "من",
        "to" => "إلى",
        "department_name" => "اسم القسم",
        "permission_list" => "صلاحيات النظام",
        "group_name" => "اسم المجموعة",
        "job_description" => "الوصف الوظيفي",
        "client_type" => "نوع العميل",
        "record_number" => "رقم السجل التجاري",
        "tax_number" => "الرقم الضريبي",
        "bank" => "البنك",
        "account_number" => "رقم الحساب البنكي",
        "identity_number" => "رقم الهوية",
        "birth_date" => "تاريخ الميلاد",
        "phone" => "رقم الجوال",
        "attachment_type" => "نوع المرفق",
        "attachement_address" => "عنوان المرفق",
        "files" => "الملفات",
        "register_type" => "نوع التسجيل",
        "country_code" => "الكود",
        "job_name" => "اسم الوظيفة",
        "contract_type" => "نوع العقد",
        "daily_transactions" => "المعاملات اليومية",
        "add_identity" => "إدراج هوية",
        "identity_source" => "مصدر الهوية",
        "select_date" => "يوم/شهر/سنة",
        "ID issuance date" => "تاريخ صدور الهوية",
        "address" => "العنوان",
        "nationality" => "الجنسية",
        "add_source_identity" => "أدخل مصدر الهوية",
        "add_address" => "أدخل العنوان",
        "add_email" => "أدخل البريد الالكترونى",
        "saudi" => "سعودي",
        "martial_status" => "الحالة الأجتماعية",
        "choose_martial_status" => "أختر الحالة الاجتماعية",
        "gender" => "النوع",
        "male" => "ذكر",
        "female" => "انثي",
        "wtire_note" => "كتابة ملاحظة",
        "add_bank" => "اضافة بنك",
        "bank_type" => "نوع البنك",
        "code" => "شفرة",
        "discounts" => "نسبة الخصم"
    ],
    "table" => [
        "#" => "#",
        "id" => "ID",
        "jobName" => "اسم الوظيفة",
        "description" => "الوصف",
        "actions" => "العمليات",
        "page_name" => "اسم الصفحة",
        "name" => "الاسم",
        "email" => "البريد الإلكتروني",
        "status" => "الحالة",
        "created_at" => "تاريخ الإنشاء",
        "createdBy" => "منشأ بواسطة",
        "create_date" => "تاريخ الإنشاء",
        "mobile" => "رقم الجوال",
        "icon" => "الأيقونة",
        "category" => "التخصص",
        "location" => "الموقع",
        "doneBy" => "تم بواسطة",
        "actionDate" => "تاريخ الإجراء",
        "archivedDate" => "تاريخ الأرشفة",
        "activity" => "النشاط",
        "reason" => "السبب",
        "main_department" => "القسم الرئيسي",
        "department_name" => "اسم القسم",
        "job_name" => "اسم الوظيفة",
        "department" => "القسم",
        "employee_name" => "اسم الموظف",
        "type" => "النوع",
        "user_name" => "اسم المستخدم",
        "user_number" => "رقم المستخدم",
        "permission_name" => "اسم الصلاحية",
        "addedBy" => "تم بواسطة",
        "by_employee" => "تم بواسطة",
        "activity_date" => "تاريخ النشاط",
        "activity_name" => "النشاط",
        "activity_reason" => "السبب",
        "group_name" => "اسم المجموعة",
        "users_number" => "عدد المستخدمين",
        "main_prog" => "البرنامج الرئيسي",
        "sub_prog" => "البرنامج الفرعي",
        "added_by" => "أضيفت بواسطة",
        "bank" => "البنك",
        "account_status" => "حالة الحساب البنكي",
        "transactions_form" => "عدد المعاملات من",
        "transactions_to" => "عدد المعاملات إلى",
        "client_period_from" => " الفترة من",
        "client_period_to" => "الفتره إلى",
        "order_number" => "رقم الطلب",
        "client_name" => "اسم العميل",
        "client_type" => "نوع العميل",
        "record_number" => "رقم السجل",
        "tax_number" => "الرقم الضريبي",
        "bank_name" => "البنك التابع له",
        "transaction_done" => "عدد المعاملات المنجزة",
        "client_status" => "الحالة",
        "order_data_form" => "تاريخ الطلب من",
        "order_data_to" => "تاريخ الطلب إلى",
        "employee" => "الموظف",
        "main_program" => "البرنامج الرئيسي",
        "sub_program" => "البرنامج الفرعي",
        "manager_name" => "اسم المدير",
        "identity_number" => "رقم الهوية",
        "birth_date" => "تاريخ الميلاد",
        "date_time" => "تاريخ / وقت",
        "ip" => "رقم معرف الجهاز",
        "archive_date" => "تاريخ الأرشفة",
        "activated_card" => "البطاقة المفعلة",
        "card_endDate" => "تاريخ إنتهاء البطاقة",
        "register_date" => "تاريخ التسجيل",
        "transaction_number" => "رقم المعاملة",
        "transaction_date" => "تاريخ المعاملة",
        "transaction_value" => "قيمة المعاملة",
        "receipt_value" => "قيمة الفاتورة",
        "rewards" => "المكافآت",
        "transaction_type" => "نوع المعاملة",
        "transaction_status" => "حالة المعاملة",
        "active_identity_card" => "البطاقة المفعلة",
        "name_of_bank" => "اسم البنك",
        "code" => "الكود",
        "branch_name" => "اسم الفرع",
        "transfer_cost" => "قيمة تكلفة التحويل",
        "transaction_count" => "عدد المعاملات",
        "ib" => "عنوان IP",
        "basic_discount" => "البطاقة الاساسية",
        "golden_discount" => "البطاقة الذهبية",
        "platinum_discount" => "البطاقة البلاتينية",
        "card_end_at" => "تاريخ انتهاء البطاقة",
        "phone" => "رقم الجوال"
    ],
    "icon" => [
        "delete" => "Delete",
        "show" => "Show",
        "edit" => "Edit",
        "download" => "Download"
    ],
    "alertBody" => [
        "archive" => "هل تريد إتمام عملية الأرشفة؟",
        "confirmAccount" => "هل تريد إتمام عملية تأكيد الحساب؟",
        "hangAccount" => "هل تريد إتمام عملية التعليق؟",
        "confirm" => "هل تريد إتمام عملية الحفظ ؟",
        "confirmSubmit" => "هل تريد إتمام عملية الحفظ ؟",
        "back" => "هل تريد العودة دون الحفظ ؟"
    ],
    "edit_static_pages" => "Edit",
    "label" => [
        "active" => "مفعل",
        "inActive" => "InActive",
        "deactive" => "تعطيل",
        "archive" => "أرشيف",
        "edit" => "تعديل",
        "add" => "إضافة",
        "reset password" => "إعادة تعيين كلمة المرور",
        "deactivated" => "تعطيل",
        "restore" => "استعادة",
        "inactive" => "معطل",
        "confirmDone" => "تم تأكيد الحساب البنكي",
        "confirmReject" => "لم يتم تأكيد الحساب البنكي",
        "confirmReview" => "تم مراجعة الحساب البنكي",
        "permanent" => "تعطيل دائم",
        "temporary" => "تعطيل لفترة",
        "success" => "ناجحة",
        "active_status" => "نشيط"
    ],
    "cards" => [
        "departments" => "الأقسام",
        "vacant_jobs" => "الوظائف الشاغرة",
        "employees" => "العملاء",
        "permanent_banned_users" => "المستخدمين المعطلين دائما",
        "temporary_banned_users" => " المستخدمين المعطلين لفترة",
        "active_users" => "المستخدمين المفعلين",
        "unvacant_jobs" => "الوظائف المشغولة",
        "archived_departments" => "الأقسام المؤرشفة",
        "active_jobs" => "الوظائف المفعلة",
        "unactive_jobs" => "الوظائف المعطلة"
    ],
    "routes" => [
        "home" => "الرئيسية",
        "departments_management" => "إدارة الأقسام",
        "control_panel" => "لوحة التحكم",
        "department_list" => "قائمة الأقسام",
        "record" => "السجل",
        "department_record" => "سجل الأقسام",
        "job_record" => "سجل الوظائف",
        "departments" => "الأقسام",
        "add" => "إضافة",
        "archives" => "الأرشيف",
        "department_archive" => "أرشيف الأقسام",
        "jobs" => "الوظائف",
        "users" => "المستخدمين",
        "permissions" => "الصلاحيات",
        "department_record_details" => "عرض قسم",
        "job_record_details" => "تفاصيل سجل الوظائف",
        "user_history" => "سجل المستخدمين",
        "permissionsRecord" => "سجل الصلاحيات",
        "users_history" => "سجل المستخدمين",
        "user_view" => "عرض مستخدم",
        "edit" => "تعديل",
        "editGroup" => "تعديل مجموعة",
        "addGroup" => "إضافة مجموعة",
        "job_view" => "عرض وظيفة",
        "users_record" => "سجل المستخدمين",
        "jobs_record" => "سجل الوظائف",
        "departments_record" => "سجل الأقسام",
        "permissions_record" => "سجل الصلاحيات",
        "viewGroup" => "عرض مجموعة",
        "permission_record" => "سجل الصلاحيات",
        "clients" => "العملاء",
        "clients_record" => "سجل العملاء",
        "clients_record_details" => "عرض العميل",
        "register_client" => "تسجيل عميل",
        "bank_account" => "طلبات فتح حساب",
        "add_group" => "إضافة مجموعة",
        "add_user" => "إضافة مستخدم",
        "add_department" => "إضافة قسم",
        "add_job" => "إضافة وظيفة",
        "follow_up" => "المتابعة",
        "activity_log" => "سجل النشاطات",
        "departments_archive" => "ارشيف الاقسام",
        "jobs_archives" => "ارشيف الوظائف",
        "citizens" => "المستخدمين",
        "transactions" => "المعاملات",
        "transaction-list" => "سجل المعاملات",
        "transactions_record" => "سجل المعاملات",
        "bank_record" => "سجل البنوك",
        "banks" => "البنوك",
        "add_bank" => "اضافة بنك",
        "edit_bank" => "تعديل بنك",
        "bank_view" => "عرض بنك",
        "discount_rates" => "نسب خصم البطاقات",
        "card_packages_record" => "سجل نسب خصم البطاقات",
        "add_card_packages" => "اضافة نسب خصم البطاقات",
        "edit_card_packages" => "تعديل نسب خصم البطاقات",
        "rasid" => "رصيد",
        "rasid_pay" => "رصيد باي",
        "transaction_view" => "عرض معاملة",
        "login" => "تسجيل دخول",
        "reset" => "كلمة المرور",
        "verification-method" => "إعادة تعيين كلمة المرور",
        "verify-code" => "كود التفعيل",
        "Page401" => "خطأ 401",
        "Page404" => "خطأ 404",
        "view" => "عرض",
        "dapertment_archive" => "ارشيف الأقسام",
        "job_archive" => "ارشيف الوظائف"
    ],
    "without" => "بدون",
    "no_data" => " لا توجد نتائج متاحة",
    "show" => "عرض",
    "Show" => "عرض",
    "active" => "مفعل",
    "deactive" => "معطل",
    "vacant" => "شاغرة",
    "occupied" => "مشغولة",
    "do_you_want_to_archive" => "هل تريد أرشفة هذا العنصر؟",
    "do_you_want_to_delete" => "Do you want to delete this item?",
    "buttons" => [
        "save" => "Save",
        "back" => "Back"
    ],
    "are_you_sure" => "Are You sure ?",
    "occupid" => "مشغول",
    "tooltip" => [
        "archive" => "أرشفة",
        "edit" => "تعديل",
        "view_details" => "عرض ",
        "messages" => "الرسائل",
        "Settings" => "الإعدادات",
        "logout" => "تسجيل خروج",
        "confirm_account" => "تأكيد الحساب",
        "confirmed_account" => "الحساب مؤكد",
        "hang_account" => "تعليق الحساب",
        "restore" => "استعادة",
        "force_delete" => "حذف "
    ],
    "main_information" => "المعلومات الرئيسية",
    "optional_information" => "معلومات اختيارية",
    "history_information" => "الحركة التاريخية",
    "archive" => "أرشيف",
    "select" => [
        "all" => "الجميع",
        "active" => "مفعل",
        "deactive" => "معطل",
        "vacant" => "شاغر",
        "occupied" => "مشغول",
        "activeF" => "مفعلة",
        "deactiveF" => "معطلة"
    ],
    "selectAll" => "اختر الكل",
    "is_active" => "مفعل",
    "in_active" => "معطل",
    "Temporary Disable" => "معطل لفترة",
    "permanent disable" => "معطل دائم",
    "un_active" => "معطل",
    "required" => "هذا الحقل مطلوب",
    "Groups" => "مجموعة صلاحيات",
    "pdf" => "PDF ",
    "xlsx" => "EXCEL ",
    "loading" => "تحميل",
    "chips" => [
        "edit" => "تعديل",
        "active" => "تفعيل",
        "deactive" => "تعطيل",
        "add" => "إضافة",
        "archive" => "أرشفة",
        "restored" => "استعادة"
    ],
    "image_extentions" => "png-hpg-jpeg",
    "active_f" => "مفعلة",
    "in_active_f" => "معطلة",
    "vacant_f" => "شاغرة",
    "occupied_f" => "مشغولة",
    "all" => "الجميع",
    "occupid_f" => "مشغولة",
    "fullname" => "الاسم الكامل",
    "gender" => "الجنس",
    "phone" => "رقم الجوال",
    "whatsapp" => "واتس أب",
    "login_id" => "رقم المستخدم",
    "department" => "القسم",
    "created_at" => "أنشئت في",
    "user_type" => "نوع المستخدم",
    "ban_status" => "حالة الحظر",
    "addedBy" => "أضيفت بواسطة",
    "searched" => "مبحوث",
    "activated" => "تفعيل",
    "activity_details" => "تفاصيل النشاط",
    "created" => "إضافة",
    "updated" => "تعديل",
    "destroy" => "ارشفة",
    "restored" => "استعادة",
    "permanent_delete" => "تعطيل",
    "deactivated" => "تعطيل",
    "unarchive_job" => "لا يمكن ارشفة وظيفة مشغولة",
    "permission_name_error_used_before" => "عفوا اسم المجموعة موجود مسبقاً",
    "un_archive_has_sub_department" => "لا يمكن ارشفة هذا القسم لأنة يتحوي علي فسم فرعي",
    "user_name" => " أسم المستخدم",
    "department_used_before" => "اسم القسم مستخدم بالفعل",
    "without_main_department" => "بدون قسم رئيسي",
    "is_active_new" => "تفعيل",
    "without_department" => "بدون قسم",
    "without_description" => "بدون وصف",
    "select_all" => "اختر الكل",
    "cant_archive_department_has_job" => "لا يمكن أرشفة قسم مرتبط بوظائف",
    "company" => "شركات",
    "Institution" => "مؤسسة",
    "member" => "عضو",
    "freelance_doc" => "مستقل",
    "famous" => "مشهور",
    "other" => "أخري",
    "delegate" => "مفوض",
    "direct" => "مباشر",
    "identity" => "هوية",
    "docs" => "المستندات",
    "images" => "صور",
    "videos" => "فيديو",
    "voices" => "صوتيات",
    "you_want_to_restore" => "هل تريد اتمام عملية الأستعادة؟",
    "pending" => "تحت الانتظار",
    "before_review" => "قبل المراجعة",
    "reviewed" => "تم المراجعة",
    "male" => "ذكر",
    "female" => "أنثي",
    "married" => "متزوج",
    "single" => "أعزب",
    "force_delete" => "هل تريد اتمام عملية الحذف النهائي؟",
    "duplicateAttachment" => "لا يمكن تكرار نوع  المرفق",
    "add_new_branch" => "اضافة فرع جديد",
    "payment" => "دفع",
    "bank_transaction" => "المعاملات المصرفية",
    "receive_credit" => "الحصول على الائتمان",
    "recharge_credit" => "إعادة شحن الرصيد",
    "upgrade_card" => "بطاقة الترقية",
    "wallet_transfer" => "تحويل المحفظة",
    "cancel" => "تم الالغاء",
    "fail" => "فاشلة",
    "received" => "تم الاستلام",
    "success" => "ناجحة",
    "delivared" => "تم الأستلام",
    "sar" => "ر.س",
    "forget_password" => "نسيت كلمة المرور",
    "all_rights_reserved_by_rasid_pay" => "جميع الحقوق محفوظة الفنتك للتكنولوجيا والمعلومات"
];
