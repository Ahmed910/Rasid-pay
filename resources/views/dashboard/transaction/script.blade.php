    ...
    $(function () {
      ...
      var table = $("#transactionsTable").DataTable({
        ...
        columns: [{
          ...
          {
            data: function (data) {
              if (data.status === "{{ trans('dashboard.transaction.status_cases.success') }}") {
                return ` <span class="badge bg-success-opacity py-2 px-4">${data.status}</span>`;
              } else if (data.status === "{{ trans('dashboard.transaction.status_cases.fail') }}") {
                return ` <span class="badge bg-danger-opacity py-2 px-4">${data.status}</span>`;
              } else if (data.status === "{{ trans('dashboard.transaction.status_cases.pending') }}") {
                return ` <span class="badge bg-warning-opacity py-2 px-4">${data.status}</span>`;
              } else if (data.status === "{{ trans('dashboard.transaction.status_cases.received') }}") {
        ...
        "drawCallback": function (settings, json) {
          ...
          for (var i = 0; i < transactionTableSorting.length; i++) {
            transactionTableSorting[i].innerText = transactionTableSorting[i].innerText.replace(
          ...
          for (var i = 1; i < transactionTablePagination.length - 1; i++) {
            transactionTablePagination[i].innerText = transactionTablePagination[i].innerText.replace(
              transactionTablePagination[i].innerText, transactionTablePagination[i].innerText
                .toArabicUni());
          }
          // info
          var transactionTableInfo = document.getElementById('transactionsTable_info').innerText;
          document.getElementById('transactionsTable_info').innerText = transactionTableInfo.replace(
      ...
      $('#card_package_id').on('select2:select', function (e) {
        table.draw();
      });
