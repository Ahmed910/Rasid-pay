    ...
    <form method="post" action="{{ route('dashboard.post_login') }}" class="needs-validation" id="login-form" novalidate>
        ...
        <div class="form-group">
            ...
            <input
                ...
                type="number" pattern="^[1-9]\d*$" onkeypress="return /[0-9a-zA-Z]/i.test(event.key)" maxlength="10"
                class="form-control stop-copy-paste number-regex @error('username')
is-invalid
@enderror" id="userId"
                name="username" value="{{ old('username') }}" placeholder="{{ trans('auth.userID') }}" />
        ...
        <div class="form-group" id="password">
            ...
            <div class="input-group" id="show_hide_password">
                <input class="form-control stop-copy-paste @error('password') is-invalid @enderror"
                    placeholder="{{ trans('auth.password') }}" type="password" name="password" />
                @error('password')
        ...
        <a onclick="submitForm('#login-form')"
            class="btn btn-primary w-100 mt-5 a-submit">{{ trans('auth.login_title') }}</a>
